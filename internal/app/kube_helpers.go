...
func createServiceAccount(saName string, namespace string) (bool, string) {
	...
	if exitCode != 0 {
		//log.Fatal("failed to create service account " + saName + " in namespace [ " + namespace + " ]: " + err)
		return false, err
	}
...
func createRoleBinding(role string, saName string, namespace string) (bool, string) {
	...
	log.Info("Creating " + resource + " for service account [ " + saName + " ] in namespace [ " + namespace + " ] with role: " + role + ".")
	cmd := command{
		Cmd:         "kubectl",
