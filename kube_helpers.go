...
func createRBAC(sa string, namespace string, role string, roleConfigFile string) (bool, string) {
	var ok bool
	var err string
	if role == "" {
	...
	if ok, err = createServiceAccount(sa, namespace); ok {
		...
		if ok, err = createRole(namespace, role, roleConfigFile); ok {
			if ok, err = createRoleBinding(role, sa, namespace); ok {
				return true, ""
			}
			return false, err
...
func createRole(namespace string, role string, roleConfigFile string) (bool, string) {
	var resource []byte
	var e error

	if roleConfigFile != "" {
		// load file from path of TillerRoleConfigFile
		resource, e = ioutil.ReadFile(roleConfigFile)
	} else {
